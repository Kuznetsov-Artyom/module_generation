...
void testType(int countValues = 1)
{
    ...
    else if (genRandData::is_float_point_t<T>::value) {std::cout << "float point type\n";}
    else if (genRandData::is_char_t<T>::value) {std::cout << "char type\n";}
    else
    {
    ...
    genRandData::Generator<T> generator(std::numeric_limits<T>::min(), std::numeric_limits<T>::max());
